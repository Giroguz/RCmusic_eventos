...
app.use(express.json()); const supabaseProxyUrl = process.env.SUPABASE_URL || 'https://fzqpmpgbubpmongodcat.supabase.co' const supabaseProxyKey = process.env.SUPABASE_ANON_KEY || ''; async function callSupabaseRpc(name, body) {   const response = await fetch(`${supabaseProxyUrl}/rest/v1/rpc/${name}`, { method: 'POST', headers: { apikey: supabaseProxyKey, Authorization: `Bearer ${supabaseProxyKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) })   const data = await response.json().catch(() => null)   if (!response.ok) throw new Error(data?.message || data?.error_description || 'Supabase request failed')   return data; } app.post('/api/dj/login', async (req, res) => {   try { const data = await callSupabaseRpc('dj_login', { p_email: String(req.body?.email || '').trim().toLowerCase(), p_code: String(req.body?.code || '').trim() }); res.json(data) } catch (error) { res.status(400).json({ error: error.message }) } }); app.post('/api/dj/events', async (req, res) => {   try { const data = await callSupabaseRpc('dj_get_events', { p_token: String(req.body?.token || '') }); res.json(data) } catch (error) { res.status(400).json({ error: error.message }) } }); app.post('/api/dj/access', async (req, res) => {   try { const data = await callSupabaseRpc('dj_check_access', { p_token: String(req.body?.token || '') }); res.json(data) } catch (error) { res.status(400).json({ error: error.message }) } })
