...
export default function App() {
  const [screen, setScreen] = useState(() => { try { return window.location.hash.slice(1) || window.history.state?.screen || readRouteStack().at(-1)?.screen || 'home' } catch { return 'home' } })
  const screenRef = useRef(screen)
  const [activeEvent, setActiveEvent] = useState(() => { try { return window.history.state?.activeEvent || readRouteStack().at(-1)?.activeEvent || null } catch { return null } })
