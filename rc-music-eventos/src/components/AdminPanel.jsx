...
import { Check, CheckCircle2, Clock3, FileCheck2, Lock, Plus, RefreshCw, ShieldCheck, UserCheck, UserX, X, XCircle } from 'lucide-react'
...
export default function AdminPanel({ session, onClose }) {
  ...
  async function reviewProof(id, status) {
    ...
    try { await adminReviewSubscriptionProof(id, status, status === 'approved' ? 'Pago verificado por el desarrollador.' : 'Comprobante rechazado por el desarrollador.', session.token); setProofs((current) => current.filter((proof) => proof.id !== id)); setNotice(status === 'approved' ? 'Pago aprobado y plan activado.' : 'Comprobante rechazado.'); await load() } catch { setError('No se pudo actualizar el comprobante.') } finally { setProofBusy(false) }
  }
  ...
  return <div className="fixed inset-0 z-[100] isolate overflow-y-auto overscroll-contain bg-ink p-4"><div className="mx-auto my-6 max-w-6xl rounded-[2rem] border border-white/10 bg-ink p-5 sm:p-8">
    ...
    <div className="space-y-3">{djs.filter((dj) => dj.role !== 'admin').map((dj) => <div key={dj.id} className="grid gap-4 rounded-2xl border border-white/10 bg-white/[.03] p-4 lg:grid-cols-[minmax(0,1fr)_220px_auto]">
      <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-bold">{dj.displayName}</p><span className="break-all text-xs text-white/45">{dj.email}</span><p className="mt-1 break-all text-xs text-turquoise/80">Código: <strong>{dj.generatedCode || 'No disponible · regenerar'}</strong></p><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${dj.isActive ? 'bg-emerald-400/15 text-emerald-200' : dj.approved ? 'bg-amber-300/15 text-amber-200' : 'bg-white/10 text-white/45'}`}>{dj.isActive ? t('active') : dj.approved ? 'Autorizado · vencido' : 'Pendiente'}</span><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${dj.emailVerified || dj.email_verified ? 'bg-emerald-400/15 text-emerald-200' : 'bg-amber-300/15 text-amber-100'}`}>{dj.emailVerified || dj.email_verified ? '✓ Correo verificado' : 'Correo pendiente'}</span></div><p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-white/45"><span>Plan: <strong className="text-white/70">{planLabel(dj.planType, t)}</strong></span><span className="inline-flex items-center gap-1"><Clock3 size={13} /> <LiveCountdown expiresAt={dj.planExpiresAt} /></span></p><p className="mt-1 text-[11px] text-white/35">{dj.approved ? 'Acceso autorizado' : 'Acceso no autorizado'} · {dj.daysRemaining || 0} {t('daysRemaining')}</p></div>
