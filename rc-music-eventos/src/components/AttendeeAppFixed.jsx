...
export default function AttendeeApp({ event, onExit }) {
  ...
  useEffect(() => {
    const onBack = (event) => setChatOpen(event.state?.rcMusicOverlay === 'chat')
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8 text-white">
    ...
    <section className="glass rounded-3xl p-5">
      ...
      <form onSubmit={search} className="flex gap-2">
        <input value={query} onChange={(e) => setQuery(e.target.value)} className="input-dark flex-1" placeholder="Artista o canción" />
        <button disabled={loading} className="btn-primary">{loading ? 'Buscando…' : 'Buscar'}</button>
      </form>
      <div className="mt-5 space-y-2">{results.map((track) => <div key={`${track.id}-${track.title}`} className="rounded-xl border border-white/10 p-3"><strong>{track.title}</strong><p className="text-sm text-white/60">{track.artist}</p></div>)}</div>
    </section>
