...
export default function ChatRoom({ eventId, role = 'attendee', onClose }) {
  ...
  useEffect(() => subscribeToEventPresence(eventId, role, setOnline, 'chat', true), [eventId, role])
  useEffect(() => {
  ...
  async function send(e) {
    ...
    const channel = supabase.channel(`event-chat-${eventId}`)
    await channel.send({ type: 'broadcast', event: 'message', payload: { id: `${Date.now()}-${Math.random()}`, sender, role, text: clean, createdAt: new Date().toISOString() } })
