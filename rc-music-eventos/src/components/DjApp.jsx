...
export default function DjApp({ onExit, session }) {
  ...
  async function searchDriveForRequest(request) {
    if (!request?.videoId || downloadingId) return
    setDownloadingId(request.id)
    try {
      const matches = await searchDriveAudio(`${request.title} ${request.artist}`)
      if (!matches.length) {
