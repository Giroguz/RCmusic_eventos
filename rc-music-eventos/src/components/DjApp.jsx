...
function DjRequestRow({ request, onStatus, onPreview, onProof, onDownload, downloading, readOnly, t }) {
...
export default function DjApp({ onExit, session }) {
  ...
  async function downloadRequestAudio(request) {
    ...
    try {
      ...
      if (matches.length === 1) await downloadDriveAudio(matches[0].id, matches[0].name)
      else await downloadYoutubeAudio(request.videoId, 'mp3', `${request.title}-${request.artist}`, `${request.title} ${request.artist}`)
