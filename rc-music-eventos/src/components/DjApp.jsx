...
import { createDjEvent, getDjAccess, getDjEventQr, getDjEvents, setRequestStatus, signOutDj, supabaseEnabled, updateDjEventInfo, updateDjEventQr } from '../lib/supabase'
...
export default function DjApp({ onExit, session }) {
  ...
  const [profileForm, setProfileForm] = useState({ djName: '', yapeNumber: '', contact: '', qrImage: '' })
  ...
  useEffect(() => {
    if (activeEvent) setProfileForm({ djName: activeEvent.djName || '', yapeNumber: activeEvent.yapeNumber || '', contact: activeEvent.contact || '', qrImage: activeEvent.qrImage || '' })
  ...
  async function saveProfile(e) {
    ...
    const updated = { ...activeEvent, djName: profileForm.djName.trim() || 'DJ', yapeNumber: profileForm.yapeNumber.trim(), contact: profileForm.contact.trim(), qrImage: profileForm.qrImage || '' }
    try {
      ...
      setEvents((current) => current.map((event) => event.id === activeEvent.id ? { ...event, djName: saved.djName, yapeNumber: saved.yapeNumber, contact: saved.contact, qrImage: updated.qrImage } : event))
  ...
  function downloadPlayedSongs() {
    ...
    const songLines = playedRequests.map((request, index) => `${index + 1}.-Canción: ${request.title}`)
