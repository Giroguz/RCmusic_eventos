...
function DjRequestRow({ request, onStatus, onPreview, onProof, onDriveSearch, onDriveCancel, searchingDrive, readOnly, t }) {
  ...
  return <div className={`flex flex-col gap-4 border-b border-white/10 px-4 py-4 last:border-0 sm:flex-row sm:items-center ${played || rejected ? 'opacity-55' : ''}`}><button onClick={() => onPreview(request)} className="group relative h-16 w-full shrink-0 overflow-hidden rounded-xl bg-white/10 sm:w-24"><MediaThumbnail src={request.thumbnail} alt={request.title} className="h-full w-full object-cover" /><span className="absolute inset-0 grid place-items-center bg-black/30 opacity-0 transition group-hover:opacity-100"><Play size={18} fill="currentColor" /></span></button><div className="min-w-0 flex-1"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className={`truncate font-bold ${played || notFound || rejected ? 'line-through decoration-white/60' : ''}`}>{request.title}</p><p className="truncate text-xs text-white/50">{request.artist}</p></div><span className="flex items-center gap-1 rounded-lg bg-neon/10 px-2 py-1 text-xs font-extrabold text-neon">{request.likes} <span className="font-normal text-neon/60">likes</span></span></div><div className="mt-2 space-y-1 text-xs text-white/45"><p><span className="font-semibold text-white/65">{t('requestedBy')}:</span> {request.requester || '—'}</p><p><span className="font-semibold text-white/65">{t('dedicatedTo')}:</span> {request.dedication || '—'}</p><p className={`${statusTone} font-bold`}><span>{t('statusLabel')}:</span> {statusText}</p>{request.paymentProof && <button onClick={() => onProof(request.paymentProof)} className="font-semibold text-turquoise underline-offset-2 hover:underline">{t('viewPaymentProof')}</button>}</div></div><div className="flex w-full flex-wrap gap-2 sm:w-auto">{searchingDrive ? <button onClick={onDriveCancel} className="flex w-full shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-red-300/30 bg-red-400/10 px-3 py-2.5 text-xs font-bold text-red-200 transition hover:bg-red-400/20 sm:w-auto" title={t('cancel')}><LoaderCircle size={15} className="animate-spin" /> <span>{t('searching')}</span></button> : <button onClick={() => onDriveSearch(request)} className="flex w-full shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl bg-turquoise/10 px-3 py-2.5 text-xs font-bold text-turquoise transition hover:bg-turquoise/20 sm:w-auto" title={t('searchDrive')}><Search size={15} /> <span>{t('searchDrive')}</span></button>}{!readOnly && (awaiting ? <><button onClick={() => onStatus(request.id, 'pending')} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-400/15 px-3 py-2.5 text-xs font-bold text-emerald-200 transition hover:bg-emerald-400/25 sm:flex-none" title={t('approvePayment')}><CheckCircle2 size={15} /><span>{t('approvePayment')}</span></button><button onClick={() => onStatus(request.id, 'payment-rejected')} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-amber-400/10 px-3 py-2.5 text-xs font-bold text-amber-200 transition hover:bg-amber-400/20 sm:flex-none" title={t('rejectPayment')}><CircleAlert size={15} /><span>{t('rejectPayment')}</span></button></> : <div className="grid w-full grid-cols-3 gap-2 sm:flex sm:w-auto"><button onClick={() => onStatus(request.id, 'played')} className={`flex min-w-0 items-center justify-center gap-1 rounded-xl px-2 py-2.5 text-[11px] font-bold transition sm:flex-none sm:px-3 sm:text-xs ${played ? 'bg-[#b8ff3d]/25 text-[#b8ff3d] shadow-[0_0_14px_rgba(184,255,61,.2)]' : 'bg-[#b8ff3d]/10 text-[#b8ff3d] hover:bg-[#b8ff3d]/20'}`} title="Tocada"><CheckCircle2 size={14} /><span className="truncate">Tocada</span></button><button onClick={() => onStatus(request.id, 'not-found')} className={`flex min-w-0 items-center justify-center gap-1 rounded-xl px-2 py-2.5 text-[11px] font-bold transition sm:flex-none sm:px-3 sm:text-xs ${notFound ? 'bg-[#ffe600]/25 text-[#ffe600] shadow-[0_0_14px_rgba(255,230,0,.2)]' : 'bg-[#ffe600]/10 text-[#ffe600] hover:bg-[#ffe600]/20'}`} title="No ubicada"><CircleAlert size={14} /><span className="truncate">No ubicada</span></button><button onClick={() => onStatus(request.id, 'payment-rejected')} className={`flex min-w-0 items-center justify-center gap-1 rounded-xl px-2 py-2.5 text-[11px] font-bold transition sm:flex-none sm:px-3 sm:text-xs ${rejected ? 'bg-[#ff3b5f]/25 text-[#ff3b5f] shadow-[0_0_14px_rgba(255,59,95,.2)]' : 'bg-[#ff3b5f]/10 text-[#ff3b5f] hover:bg-[#ff3b5f]/20'}`} title="Rechazada"><X size={14} /><span className="truncate">Rechazada</span></button></div>)} </div></div>
}
...
export default function DjApp({ onExit, session = getStoredDjSession() }) {
  ...
  const [form, setForm] = useState({ name: '', contact: '', yapeNumber: '', thankYou: '', qrImage: '', tipsRequired: false })
  ...
  async function searchDriveLibrary(event) {
    ...
    try {
      ...
      setDownloadOptions({ request: null, query, matches })
  ...
  async function createEvent(e) {
    ...
    try {
      ...
      setEvents(next); if (!supabaseEnabled) saveEvents(next); setActiveId(event.id); setShowCreate(false); setForm({ name: '', contact: '', yapeNumber: '', thankYou: '', qrImage: '', tipsRequired: false }); setNotice(`${t('eventCreated')} ${event.code}`); setTimeout(() => setNotice(''), 5000)
    } catch {
