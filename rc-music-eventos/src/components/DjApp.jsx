...
import { BarChart3, Check, CheckCircle2, CircleAlert, Clock3, Download, ExternalLink, Eye, Headphones, ImagePlus, Link2, ListMusic, Music2, ShieldCheck, PauseCircle, Play, Plus, RefreshCw, Search, Settings2, Sparkles, Trash2, X } from 'lucide-react'
import { AppShell, PageContainer } from './Brand'
import { getEvents, makeCode, saveEvents } from '../lib/storage'
import { createDjEvent, finalizeDjEvent, getDjAccess, getDjEventQr, getDjEvents, setRequestStatus, signOutDj, supabaseEnabled, updateDjEventInfo, updateDjEventQr, updateDjEventTipSettings } from '../lib/supabase'
import AdminPanel from './AdminPanel'
import { useLanguage } from '../lib/i18n'
import { downloadDriveAudio, fetchDriveAudio, searchDriveAudio } from '../lib/download'
...
function DjRequestRow({ request, onStatus, onPreview, onProof, onSearchDrive, searchingDrive, readOnly, t }) {
...
export default function DjApp({ onExit, session }) {
  ...
  async function searchDriveForRequest(request) {
    if (!request || downloadingId) return
    setDownloadingId(request.id)
    try {
      const searchPromise = searchDriveAudio(`${request.title} ${request.artist}`)
      const matches = await Promise.race([searchPromise, new Promise((_, reject) => setTimeout(() => reject(new Error('DRIVE_SEARCH_TIMEOUT')), 30000))])
      if (!matches.length) {
        setNotice('No ubicada en Drive')
        return
      }
      setDownloadOptions({ request, matches })
    } catch { setNotice(t('downloadFailed')) } finally { setDownloadingId(''); setTimeout(() => setNotice(''), 4500) }
  }
  ...
  async function downloadSelectedDriveFile(file) {
    ...
    setDownloadingId(downloadOptions?.request?.id || 'drive-option')
    try { await downloadDriveAudio(file.id, file.name); setDownloadOptions(null); setNotice(t('downloadReady')) } catch { setNotice(t('downloadFailed')) } finally { setDownloadingId(''); setTimeout(() => setNotice(''), 4500) }
  }
