...
export default function DjApp({ onExit, session }) {
  ...
  async function downloadRequestAudio(request, format) {
    ...
    try { await downloadYoutubeAudio(request.videoId, format, `${request.title}-${request.artist}`); setNotice(t('downloadReady')) } catch { setNotice(t('downloadFailed')) } finally { setDownloadingId(''); setTimeout(() => setNotice(''), 4500) }
  }
