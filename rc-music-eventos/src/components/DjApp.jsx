...
export default function DjApp({ onExit, session }) {
  ...
  async function downloadRequestAudio(request) {
    ...
    try { await downloadYoutubeAudio(request.videoId, 'mp3', `${request.title}-${request.artist}`); setNotice(t('downloadReady')) } catch { setNotice(t('downloadFailed')) } finally { setDownloadingId(''); setTimeout(() => setNotice(''), 4500) }
  }
