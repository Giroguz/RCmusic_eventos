...
export default function DjApp({ onExit, session = getStoredDjSession() }) {
  ...
  useEffect(() => {
    if (activeEvent) setProfileForm({ djName: activeEvent.djName || '', yapeNumber: activeEvent.yapeNumber || '', contact: activeEvent.contact || '', thankYou: activeEvent.thankYou || '', qrImage: activeEvent.qrImage || '', tipsRequired: Boolean(activeEvent.tipsRequired) })
  }, [activeEvent?.id, activeEvent?.djName, activeEvent?.yapeNumber, activeEvent?.contact, activeEvent?.thankYou, activeEvent?.qrImage])
  ...
  async function saveProfile(e) {
    ...
    const updated = { ...activeEvent, djName: profileForm.djName.trim() || 'DJ', yapeNumber: profileForm.yapeNumber.trim(), contact: profileForm.contact.trim(), thankYou: profileForm.thankYou.trim().slice(0, 150), qrImage: profileForm.qrImage || '', tipsRequired: Boolean(profileForm.tipsRequired) }
    try {
      ...
      setEvents((current) => current.map((event) => event.id === activeEvent.id ? { ...event, djName: saved.djName, yapeNumber: saved.yapeNumber, contact: saved.contact, thankYou: updated.thankYou, qrImage: updated.qrImage, tipsRequired: updated.tipsRequired } : event))
