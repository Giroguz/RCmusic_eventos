...
import { downloadYoutubeAudio } from '../lib/download'
...
export default function DjApp({ onExit, session }) {
  ...
  async function downloadRequestAudio(request, format) {
    if (!request?.videoId || downloadingId) return
    setDownloadRequest(null)
    setDownloadingId(request.id)
    try { await downloadYoutubeAudio(request.videoId, format, `${request.title}-${request.artist}`, `${request.title} ${request.artist}`); setNotice(t('downloadReady')) } catch { setNotice(t('downloadFailed')) } finally { setDownloadingId(''); setTimeout(() => setNotice(''), 4500) }
  }
