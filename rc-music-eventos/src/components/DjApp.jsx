...
export default function DjApp({ onExit, session }) {
  ...
  if (!activeEvent) return <AppShell onHome={onExit} right={<div className="flex items-center gap-2"><EmailVerificationBadge access={access} /><AccessCountdown access={access} />{access?.role === 'admin' && <button onClick={() => setShowAdmin(true)} className="btn-secondary px-3 py-2 text-sm"><Crown size={16} /> Desarrollador</button>}</div>}><PageContainer><div className="mx-auto max-w-lg glass rounded-2xl p-6"><Headphones size={28} className="mx-auto mb-3 text-violet-200" /><p className="mb-5 text-center text-white/60">{t('noEvents')}</p><form onSubmit={createEvent} className="space-y-3"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="input-dark" placeholder={t('eventName')} /><button className="btn-primary w-full"><Plus size={17} /> {t('create')}</button></form></div>{showAdmin && access?.role === 'admin' && <AdminPanel session={access} onClose={() => setShowAdmin(false)} />}</PageContainer></AppShell>
