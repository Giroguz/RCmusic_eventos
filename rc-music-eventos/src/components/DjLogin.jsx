...
function PlanCards({ token }) {
  ...
  async function sendProof() {
    if (!token) { setProofMessage('Vuelve a ingresar con tu código para enviar el comprobante.'); return }
    if (!selectedPlan || !proofImage) { setProofMessage('Selecciona un plan y sube la foto del comprobante.'); return }
    setProofBusy(true); setProofMessage('')
    try { await submitSubscriptionProof(selectedPlan, proofImage, token); setProofMessage('Comprobante enviado. El desarrollador revisará el pago y activará tu plan.'); setProofImage('') } catch { setProofMessage('No se pudo enviar el comprobante. Inténtalo nuevamente.') } finally { setProofBusy(false) }
  }
...
export default function DjLogin({ onLogin, onBack }) {
  ...
  async function submit(e) {
    ...
    catch (err) { const message = String(err?.message || '').toLowerCase(); const expired = message.includes('plan expired') || message.includes('plan venc'); setPlanExpired(expired); setExpiredSession(expired ? err?.access || null : null); setError(err?.message === 'SUPABASE_REQUIRED' ? t('supabaseRequired') : expired ? t('planExpired') : t('loginError')) }
  }
