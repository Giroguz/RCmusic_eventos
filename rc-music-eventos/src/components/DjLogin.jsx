...
import { getStoredDjSession, getSubscriptionPlanPrices, getSubscriptionQr, getSubscriptionYapeNumber, sendEmailVerificationLink, signInDj, signOutDj, startDjTrial, submitSubscriptionProof, supabase, supabaseEnabled } from '../lib/supabase'
// Yape subscription number and plan prices are loaded from developer-controlled settings.
import { useLanguage } from '../lib/i18n'
import { PLAN_OPTIONS, mergePlanOptions, planPriceText } from '../lib/plans'
...
function PlanCards({ token, email, code }) {
  ...
  useEffect(() => { Promise.all([getSubscriptionQr(), getSubscriptionYapeNumber(), getSubscriptionPlanPrices()]).then(([qr, number, prices]) => { setSubscriptionQr(qr); setYapeNumber(number); setPlans(mergePlanOptions(prices)) }).catch(() => {}) }, [])
...
export default function DjLogin({ onLogin, onBack, developerMode = false }) {
  ...
  useEffect(() => {
    ...
    const finishRecovery = async (session) => {
      try {
        ...
        if (!pending || !session?.user?.email || session.user.email.toLowerCase() !== pending.email || !session.user.email_confirmed_at) return
        const { data, error } = await supabase.rpc('dj_recover_admin_code')
        if (error) throw error
        const result = Array.isArray(data) ? data[0] : data
        if (!active) return
        sessionStorage.removeItem('rc_pending_recovery_v1'); setRecoveryEmail(pending.email); setRecoveryCode(result.generated_code); setRecoveryMessage('Código regenerado. Guárdalo en un lugar seguro.')
      } catch (error) { if (active) setRecoveryMessage('No se pudo regenerar el código. Verifica el enlace y vuelve a intentarlo.') }
    }
  ...
  async function requestRecovery(e) {
    ...
    try {
      ...
      setRecoveryMessage('Te enviamos un enlace de verificación. Ábrelo en tu correo para generar un código nuevo.')
    } catch { setRecoveryMessage('No se pudo enviar el enlace. Comprueba el correo e inténtalo de nuevo.') }
  }
