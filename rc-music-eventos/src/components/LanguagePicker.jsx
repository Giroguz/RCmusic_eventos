...
export default function LanguagePicker({ compact = false }) {
  ...
  return <div className={`relative inline-flex min-w-0 rounded-xl border border-white/10 bg-white/[.05] text-white/65 ${compact ? 'p-1' : 'p-1.5'}`} title={t('languageLabel')}>
    ...
    {open && <div role="listbox" aria-label={t('languageLabel')} style={{ backgroundColor: '#08050d', background: '#08050d', opacity: 1, isolation: 'isolate', pointerEvents: 'auto', zIndex: 100000, boxShadow: '0 18px 40px rgba(0,0,0,.75)' }} onPointerDown={(event) => event.stopPropagation()} className="absolute right-0 top-full z-[100] mt-1.5 min-w-[168px] overflow-hidden rounded-xl border border-white/20 p-1 shadow-2xl shadow-black/70 ring-1 ring-black/40">
      {LANGUAGES.map((item) => <button type="button" role="option" aria-selected={item.code === language} key={item.code} onPointerDown={(event) => event.stopPropagation()} onPointerUp={(event) => { event.stopPropagation(); chooseLanguage(item.code) }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); chooseLanguage(item.code) } }} className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs font-semibold transition-colors ${item.code === language ? 'bg-white/10 text-white' : 'text-white/70 hover:bg-white/[.08] hover:text-white'}`}>
