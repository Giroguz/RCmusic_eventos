...
export default function LanguagePicker({ compact = false }) {
  ...
  return <div className={`relative inline-flex min-w-0 rounded-xl border border-white/10 bg-white/[.05] text-white/65 ${compact ? 'p-1' : 'p-1.5'}`} title={t('languageLabel')}>
    ...
    {open && <div role="listbox" aria-label={t('languageLabel')} className="absolute right-0 top-full z-50 mt-1.5 min-w-[168px] overflow-hidden rounded-xl border border-white/20 bg-[#130d1d] p-1 shadow-2xl shadow-black/70 ring-1 ring-black/40">
