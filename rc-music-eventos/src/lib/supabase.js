...
export async function adminListDjs(token) { const { data, error } = await supabase.rpc('admin_list_djs_with_codes', { p_token: token }); if (error) throw error; return (data || []).map(account) }
export async function adminCreateDj(input, token) { const { data, error } = await supabase.rpc('admin_create_dj_with_code', { p_token: token, p_email: input.email, p_display_name: input.displayName, p_plan_type: input.planType || 'monthly' }); if (error) throw error; return account(Array.isArray(data) ? data[0] : data) }
...
export async function adminRegenerateCode(id, token) { const { data, error } = await supabase.rpc('admin_regenerate_code_with_code', { p_token: token, p_dj_id: id }); if (error) throw error; return account(Array.isArray(data) ? data[0] : data) }
