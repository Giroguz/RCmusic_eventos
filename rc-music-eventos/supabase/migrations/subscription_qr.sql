...
alter table public.subscription_settings add column if not exists yape_number text;
...
create or replace function public.admin_get_subscription_yape_number(p_token text)
returns text language plpgsql security definer set search_path=public as $$
declare a public.dj_accounts;
begin
  select * into a from public._dj_access(p_token);
  if coalesce(a.role,'') <> 'admin' or a.email <> 'djgianfrancoromerodechosica@gmail.com' then raise exception 'Admin only'; end if;
  return (select yape_number from public.subscription_settings where id=true);
end $$;
grant execute on function public.admin_get_subscription_yape_number(text) to anon,authenticated;

create or replace function public.admin_set_subscription_yape_number(p_token text,p_yape_number text)
returns text language plpgsql security definer set search_path=public as $$
declare a public.dj_accounts; result text;
begin
  select * into a from public._dj_access(p_token);
  if coalesce(a.role,'') <> 'admin' or a.email <> 'djgianfrancoromerodechosica@gmail.com' then raise exception 'Admin only'; end if;
  if p_yape_number is not null and length(p_yape_number) > 30 then raise exception 'Yape number too long'; end if;
  update public.subscription_settings set yape_number=nullif(trim(p_yape_number),''),updated_at=now() where id=true returning yape_number into result;
  return result;
end $$;
grant execute on function public.admin_set_subscription_yape_number(text,text) to anon,authenticated;

create or replace function public.get_subscription_yape_number()
returns text language sql security definer set search_path=public as $$
  select yape_number from public.subscription_settings where id=true limit 1;
$$;
